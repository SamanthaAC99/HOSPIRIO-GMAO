...
export default function EquiposCalibracionview() {
	...
	const [calibracionEquipo, setCalibracionEquipo] = useState([]);
	...
	const [cantidadParametros,setCantidadParametros] = useState(0)
	//modals
	...
	const getData = async () => {
		...
		if (docSnap.exists()) {
			...
			setDatosCalibracion(parametros.pcalibracion)
		} else {
	...
	const DardeBaja = (_data) => {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				const ref = doc(db, "ingresocalibracion", `${_data.id}`);
	...
	const sendStorage = async (id) => {
		const storageRef = ref(storage, `inventariocalibracion/${id}`);
		try {
	...
	const mostrarModalCalibracion = (_data) => {
		setCurrentEquipo(_data)
		setCalibracionEquipo(_data.dcalibracion)
		setModalCalibracion(true);
	};
	...
	const cerrarModalCalibracion = () => {
		setModalCalibracion(false);
	};
	...
	const eliminar = (__dato) => {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				setData(equipos_salvados)
				await deleteDoc(doc(db, "ingresocalibracion", `${__dato.id}`));
				Swal.fire(
					'Equipo eliminado!',
					'',
					'success'
	...
	const sendFirestore = (_newEquipo) => {
		...
		try {
			setDoc(doc(db, "ingresocalibracion", `${_newEquipo.id}`), _newEquipo);
		} catch (e) {
	...
	const ActualizarEquipo = async () => {
		let aux_equipos = JSON.parse(JSON.stringify(data))
		const ref = doc(db, "ingresocalibracion", `${currentEquipo.id}`);
	...
	const agregarAccesorio = () => {
		...
		console.log("ACC",accesorios_declarados)
	...
	// const agregarDatoscalibracion = () => {
	// 	let calibracion_declarados = JSON.parse(JSON.stringify(currentEquipo.dcalibracion))
	// 	calibracion_declarados.push(pc1)
	// 	console.log(calibracion_declarados)
	// 	// setAccesoriosEquipo(accesorios_declarados)
	// 	// const ref = doc(db, "ingresocalibracion", `${currentEquipo.id}`);
	// 	// updateDoc(ref, {
	// 	// 	accesorios: accesorios_declarados,
	...
	const agregarDatoscalibracion = () => {
		let calibracion_declarados = currentEquipo.dcalibracion
		let varible_ejemplo=JSON.parse(JSON.stringify(pc1)) // Esto se usa para hacer una copia del objeto
		varible_ejemplo["cantidad"]=cantidadParametros
		calibracion_declarados.push(varible_ejemplo)
		console.log(calibracion_declarados)
		setCalibracionEquipo(calibracion_declarados)
		const ref = doc(db, "ingresocalibracion", `${currentEquipo.id}`);
		updateDoc(ref, {
		  dcalibracion: calibracion_declarados,
		});
		setReloadAuto(!reloadAuto)
		Swal.fire({
		  icon: 'success',
	...
	const quitarParametrocalibracion = (_par) => {
		let aux = calibracionEquipo.filter(item => item.nombre !== _par.nombre)
		const ref = doc(db, "ingresocalibracion", `${currentEquipo.id}`);
		updateDoc(ref, {
			dcalibracion: aux,
		});
		setCalibracionEquipo(aux)
	}
	...
	return (
		<>
				...
				<Grid container spacing={{ xs: 2 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                    ...
                    <Grid item xs={12} sm={12} md={2}>
                        ...
                        <Grid item xs={12} sm={12} md={2}>
							...
							sx={{ height: "100%" }}
								...
								onClick={() => navegarView("calibracion/equipos/declarar_parametros")}
							>
								Crear Parámetros 
							</Button>
						</Grid>
					...
					<Grid item xs={12} sm={12} md={3}>
						<Autocomplete
							...
							renderInput={(params) => <TextField {...params} label="Buscar Código" />}
						/>
					</Grid>
					<Grid item xs={12} sm={12} md={2}>
						<Button variant="contained"
										...
										return (
											<TableRow hover role="checkbox" tabIndex={-1} key={index}>
                                                ...
                                                <TableCell align="center">
													...
													<Button variant='contained' disabled = {deshabilitar2}  color='dark' onClick={() => mostrarModalCalibracion(row)}>Parametros</Button>
				...
				<TableContainer sx={{overflowY:"scroll",height:300}} component={Paper}>
							...
							<TableContainer sx={{overflowY:"scroll",height:300}} component={Paper}>
								<Table  aria-label="customized table">
									...
									<TableBody>
									{calibracionEquipo.map((row,index) => (
										<StyledTableRow key={index}>
			...
			<Modal className="{width:0px}" isOpen={modalInsertar}>
				...
				<ModalBody>
					<Grid container spacing={2}>
						...
						<Grid item xs={12}>
						<strong>Código Generado: </strong>{ubicacion.codigo + "-" + departamento.codigo + "-" + responsable.codigo + "-" + tipo.codigo + "-" + equipo.codigo + "-1-0"}
						</Grid>
						<Grid item xs={12}>
							<b>Importancia:    </b>
							<RadioGroup
			...
			<Modal isOpen={modalCalibracion}>
				<ModalHeader>
					<div><h3>Parámetros de Calibración</h3></div>
				</ModalHeader>
				<ModalBody>
					...
					<Grid container spacing={2}>
						<Grid item xs={12}>
							...
							<Autocomplete
								...
								renderInput={(params) => <TextField {...params} fullWidth label="Parámetros Calibracion" type="text" />}
							/>
						</Grid>
						<Grid item xs={12}>
						<TextField id="outlined-basic" inputProps={{ style: { textTransform: "uppercase" } }} value={cantidadParametros} type="number" fullWidth label="Cantidad" variant="outlined" onChange={(event) => { setCantidadParametros(event.target.value) }}/>
						</Grid>
						...
						<Grid item xs={12} >
							<Button
								variant="contained"
								fullWidth
								onClick={() => agregarDatoscalibracion()}
				...
				<ModalFooter>
					<Button
						variant="contained"
						onClick={() => cerrarModalCalibracion()}
					>
						Cerrar
					</Button>
				</ModalFooter>
...
const initialData = {
	...
	dcalibracion: [{ codigo: 123, nombre: "pepito" }],
	mantenimientos: [],
	mtbf: "",
	mttr: "",
	img: "",
...
const columns = [
    ...
    { id: 'parametros', label: 'Parámetros', minWidth: 100, align: 'center' },
