...
export default function ActividadesView() {
  ...
  const getData = () => {
      onSnapshot(doc(db, "informacion", "parametros"), (doc) => {
        setAccesorios(doc.data().actividades)
  ...
  const salir = () => {
    navegarView('calibracion/planmantenimiento');
};
  ...
  useEffect(() => {
    ...
    return(
      ...
      <Container>
        ...
        CREAR ACTIVIDADES DE CALIBRACIÓN
      </Typography>
      </Grid>
          <Grid item xs={12} md={2}>
              ...
              <Grid item xs={12} md={2}>
                  <Button variant="contained" sx={{ height: "100%", width: "100%" }} onClick={agregarItem}>Agregar Actividad</Button>
              </Grid>
              <Grid item xs={12} md={12}>
