...
export default function EquipoView() {
    ...
    const buscarEquipo =()=>{
        ...
        let auxEquipos = JSON.parse(JSON.stringify(equiposTotales.current));
        if(isObjectEmpty(currentEquipo) || currentEquipo === null){
           setEquipos(auxEquipos)
        }else{
            setEquipos([currentEquipo])
            setCurrentEquipo({})
        }
            ...
            <Container>
                ...
                <Grid container spacing={{ xs: 2 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                    ...
                    <Autocomplete
                        ...
                        disablePortal
                        id="combo-box-demo"
                        options={equiposTotales.current}
                        renderInput={(params) => <TextField {...params} label="Equipos por nombre" />}
						isOptionEqualToValue={(option, value) => option.codigo === value.codigo}
						getOptionLabel={(option) => {
							return option.nombre;
						}}
                        onChange={(event, newvalue) => setCurrentEquipo(newvalue)}
                        />
                    </Grid>
                    <Grid item xs={12} md={3}>
